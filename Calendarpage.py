...
class CalendarPage():
    ...
    def __init__(self,root, UserData) -> None:
        ...
        self.OrderObject = GetingOrdersAandputtingItIntoAFrame(root, UserData, self.mydb)
    ...
    def CalendarOrder(self,OrderData,rowValue,columnValue):
        ...
        ViewInDetailButton = CTkButton(self.CalendarScreen, text = "View order in detail"
                                       ,command= lambda:[self.OrderObject.LoadingOrdersInDetailFrame(self.OrderObject.ViewingCurrentOrdersInDetail(OrderData)),
                                                         self.CalendarScreen.grid_forget()]).grid(row = rowValue, column = columnValue,sticky = "s")
